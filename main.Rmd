...
f_controls <- c('sex', 'age_f', 'race_f', 'educ_f', 'born_usa', 
                'party_f', 'party_fs', 'religstr_f', 'reltrad_f', 'region_f')
...
p1 <- gglikert(data2004_i, include = all_of(q2_vars_f), sort = "ascending", add_totals = FALSE) +
...
p2 <- gglikert(data2004_i, include = all_of(q3_vars_f), sort = "ascending", add_totals = FALSE) +
  scale_x_continuous(labels = function(x) abs(x)*100, limits = c(-0.51, 1)) + 
  scale_fill_brewer(palette = "RdYlBu") +
...
p3 <- gglikert(data2004_i, include = all_of(q5_vars_f), sort = "ascending", add_totals = FALSE) + 
  scale_fill_brewer(palette = "RdYlBu") +
  scale_x_continuous(labels = function(x) abs(x)*100, limits = c(-0.51, 1)) +
...
p4 <- gglikert(data2004_i, include = all_of(q4_vars_f), sort = "ascending", add_totals = FALSE) + 
  scale_fill_brewer(palette = "RdYlBu") +
  scale_x_continuous(labels = function(x) abs(x)*100, limits = c(-0.51, 1)) +
...
```{r}
f_controls <- c('sex', 'age_f', 'race_f', 'educ_f', 'born_usa', 'party_fs', 'religstr_f', 'reltrad_f', 'region_f')
...
proj_ind <- fviz_pca_ind(pca_results, 
  ...
  scale_color_manual(
    ...
    labels = c("Republican" = "Republicains", "Independent" = "Indépendants", "Democrat" = "Démocrates")  # Labels
  ) +
  theme(plot.title = element_text(hjust = 0.5))
