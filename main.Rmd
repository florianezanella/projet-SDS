...
# Variables
q2_vars <- c('clseusa')
# q2_vars <- c('clsetown', 'clsestat', 'clseusa', 'clsenoam') # with all items
q2_vars_n <- paste0(q2_vars, "_n")

# Labels
# q2_lbls <- c('Not close at all', 'Not very close', 'Can\'t choose', 'Close', 'Very close') # original
q2_lbls <- c('Not at all', 'Not very much', 'Can\'t choose', 'Somewhat / fairly', 'Very much') # standardized
...
# q3_vars <- c(q3_vars, 'amancstr') # with all items
...
q3_lbls <- c('Not at all', 'Not very much', 'Can\'t choose', 'Somewhat / fairly', 'Very much') # standardized
...
q4_lbls <- c('Not at all', 'Not very much', 'Can\'t choose', 'Somewhat / fairly', 'Very much') # standardized
...
q5_lbls <- c('Not at all', 'Not very much', 'Can\'t choose', 'Somewhat / fairly', 'Very much') # standardized
...
```{r}
data2004 %>% kNN(k=5, imp_var = FALSE)
```
...
```{r}
# all_vars = c(q2_vars, q3_vars, q4_vars, q5_vars)
...
p1 <- gglikert(data2004, include = all_of(q2_vars_f), sort = "ascending", add_totals = FALSE) +
  # Does not work with q2_vars_f if it contains just one element
  # Change to include = all_of(q2_vars_f) if additional variables are used
...
draw_corrplot(t(pca_results$quali.sup$coord[, 1:5]), "Corrélation des régresseurs", "aux axes", "output/acp/corr_rég_axes_ACP.png", range = c(-1,1))
## Des projections sur les plans factoriels
plot.PCA(pca_results, 
         axes = c(1,2), 
         choix = "var", 
         select = "contrib 8", 
         habillage = "contrib",
  ...
  scale_color_gradient(name = "Contrib. (%)", 
                       low = "#6D9EC1", high = "#E46726") +
  theme_gray() +
  theme(plot.title = element_text(hjust = 0.5))

plot.PCA(pca_results, 
         axes = c(1,2), # project on axes 1 and 2
         choix = "ind", # project individuals
         select = "none", # draw all individuals
         unselect = 0.35, # add transparency to the points
         habillage = "contrib", # add a coloration depending on contrib
         label = "none", # do not display individuals' id
         invisible = "quali", # do not display categorical controls
...
draw_corrplot(t(pca_results$quali.sup$cos2[, 1:5]), "Qualité de représentation", "des régresseurs", "output/acp/qual_rep_régr_ACP.png", range = c(0,1))
```
